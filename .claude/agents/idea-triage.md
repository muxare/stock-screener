---
name: idea-triage
description: Triage the firewalled idea inbox (backlog/ideas/) before Gate 1. Read-only classifier that sorts captured discoveries into in-scope (refine into the pipeline) vs out-of-scope (park/archive), flags duplicates and stale ideas, and grounds each against the SAD's capabilities. Advisory — recommends; the human decides at Gate 1. Never promotes, archives, or mutates the board.
...
to keep the inbox a high-signal queue the human can act on at **Gate 1**, not a
graveyard.
...
- **In-scope → refine.** It maps to a real SAD#3 capability (name it) and isn't
  ...
  a candidate story for Gate 3. Cite the anchor.
...
Claude-Code-leverage lens / a Gate-3 enabler rather than the product pipeline.
...
next step`. End by naming the gate your output feeds (Gate 1) and the single
