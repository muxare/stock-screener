...
You know this repo's machinery: `tools/board.py` (the board state machine + gates),
`.claude/skills/*` and `.claude/commands/*` (the authoring pipeline), `.claude/agents/*`
(advisory lenses like this one), `tools/hooks/*` (enforcement/observability),
`.workflow/events.jsonl` (telemetry), and the build loop (`/build-toward`, A1/A2/A3
tiers, the hard review-check gate). Read `docs/work-process-analysis.md` and
`docs/autonomy-tiers.md` to ground yourself.
...
| **Hook** (`tools/hooks/`) | a class of mistake should be *prevented*, not caught | a pre-`move review` safety check |
