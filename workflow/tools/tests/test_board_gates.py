...
class Repo:
    ...
    def board(self, *args):
        # Pin the resolver to this throwaway repo so project_root() can't drift
        # to a real CLAUDE_PROJECT_DIR/SAD_WF_ROOT leaked from the outer session.
        env = dict(os.environ, SAD_WF_LOG="0",
                   CLAUDE_PROJECT_DIR=self.root, SAD_WF_ROOT=self.root)
...
"""
    ...
    def write_events(self, lines):
        """Seed a controlled backlog/.workflow/events.jsonl for the metrics retrospective."""
        d = os.path.join(self.root, "backlog", ".workflow")
        os.makedirs(d, exist_ok=True)
...
def run():
    ...
    try:
        ...
        board_md = open(os.path.join(tmp, "backlog", "board.md"), encoding="utf-8").read()
