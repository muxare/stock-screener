...
- Check each acceptance criterion explicitly; tick the boxes you actually met
  via `python workflow/tools/board.py check <id> --criterion "<substring>"` (the edit
...
3. **Pre-flight inspect (optional):** `python workflow/tools/board.py review-check <id>`
   and paste the stdout. This is now an inspection, not the enforced gate.
4. **Move to review:** `python workflow/tools/board.py move <id> review`. The move
...
5. **Move to done** only after human review (or tier A3 policy) and with all
   acceptance boxes `[x]`: `python workflow/tools/board.py move <id> done` — this also
   re-runs the gate.
...
- `python workflow/tools/board.py move <id> in-progress`  — start work (refused if
  `sad_refs` is empty; stamps `attempts`).
- `python workflow/tools/board.py move <id> review`        — hand off for review.
- `python workflow/tools/board.py move <id> done`          — refused while any
  acceptance-criteria box is unchecked, so you can't close a lying story.
- `python workflow/tools/board.py move <id> blocked --reason "<why>"` — on a SAD
  conflict or max attempts; remembers the origin column.
- `python workflow/tools/board.py move <id> unblock`       — returns it where it came from.
...
- `python workflow/tools/board.py check <id> --criterion "<substring>"` — tick a
  criterion (`--all` for every one, `--uncheck` to clear).
- `python workflow/tools/board.py set <id> <field> <value>` — set an allowlisted field
