...
- Create stories with the CLI so they land in `todo/` with a valid skeleton:
  `python workflow/tools/board.py new --capability <id> --parent FEAT-007 --title "..."`
  ...
  changes must go through `python workflow/tools/board.py set <id> sad_refs "…"` (and
...
- After decomposing, run `python workflow/tools/board.py validate --sad <sad-id>` to
