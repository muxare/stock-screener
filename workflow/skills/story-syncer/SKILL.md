...
```bash
python workflow/tools/sync_board.py push [--dry-run]     # local → .sync/remote-state.json
python workflow/tools/sync_board.py pull [--dry-run]     # remote column → board.py move
python workflow/tools/sync_board.py status
```
...
- **Pull column changes** only via `python workflow/tools/board.py move <id> <column>`.
...
`python workflow/tools/board.py list --json`), not from frontmatter — there is no
`status` field. On pull, translate a remote column change into a
`python workflow/tools/board.py move <id> <column>` so the legal-move rules still apply.

## Push (local → remote)
1. Run `python workflow/tools/board.py validate` — abort if violations.
2. `python workflow/tools/sync_board.py push` (or MCP equivalent updating remote API).
...
2. `python workflow/tools/sync_board.py pull [--dry-run]` to preview/applies column moves.
