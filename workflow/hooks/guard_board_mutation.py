#!/usr/bin/env python3
"""Block manual kanban board mutations — use workflow/tools/board.py instead.
...
BOARD_PY = re.compile(
    r"(?:^|[\s'\"])(?:python3?\s+)?(?:\./)?(?:workflow/)?tools/board\.py\b"
)
...
def check_shell_command(command: str) -> Optional[str]:
    ...
    if touches_board_story and re.search(r"\b(mv|cp|git\s+mv)\b", cmd):
        return (
            ...
            "Use: python workflow/tools/board.py move <id> <column>"
        )
...
def check_story_edit(tool: str, file_path: str, tool_input: dict) -> Optional[str]:
    ...
    hint = (
        " Tick criteria with `python workflow/tools/board.py check <id> --criterion \"...\"` "
        "and change sad_refs with `python workflow/tools/board.py set <id> sad_refs \"...\"`. "
