...
checkpoint. Before looping, read it: `python workflow/tools/board.py batch-list`.
...
`workflow/docs/autonomy-tiers.md` for checkpoints and review-check requirements.
...
## Loop (driven entirely through workflow/tools/board.py)
```
...
documented in `workflow/docs/sprint-ceremonies.md §8`. Observation loops are always safe;
...
`python workflow/tools/board.py exceptions` — every story you blocked lands in the
