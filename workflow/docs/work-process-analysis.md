...
| **Enforcement & telemetry** | A single sanctioned mutation tool (`board.py`), three hooks that block illegal moves and log everything, and an append-only event log. The board's _state is the folder structure_ — a story's column is the directory it sits in. | `workflow/tools/board.py`, `workflow/hooks/*`, `.workflow/events.jsonl` |
...
5. **A5 — Stale-idea archival.** ✅ LANDED (2026-06-27). `board.py idea-archive
   ...
   Tests in `workflow/tools/tests/test_board_sprint.py`.
...
7. **A7 — Tests** in `workflow/tools/tests/test_board_gates.py`: `idea-new` numbering + provenance,
...
8. **A8 — `board.py render`** lists the idea inbox in `board.md` and `render-html`.
   ...
   Render tests in `workflow/tools/tests/test_board_sprint.py`.
...
(2026-06-27) — documented in `workflow/docs/sprint-ceremonies.md §8` (the cadence section).
...
15. **Mark #16–#18 landed in this document** (mirroring how Phases 1–4 were closed) and
    ...
    `/sprint-retro`), and #18 (loops/cadence, documented in `workflow/docs/sprint-ceremonies.md
...
A separate study (`workflow/docs/claude-code-api-leverage.md`) asks where the programmatic Claude
...
_Appendix — sources: `.workflow/events.jsonl` (182+ events, 2026-06-22→24); `workflow/tools/board.py`,
`workflow/hooks/*`, `.claude/settings.json`; `.claude/skills/{idea-refiner,poc-to-plan,sad-author,
...
`workflow/docs/autonomy-tiers.md`. Analysis performed by four parallel read-only agents._
