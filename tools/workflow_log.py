...
Events are written to `.workflow/events.jsonl` at the repo root. Disable with
SAD_WF_LOG=0. Use `python tools/board.py logs` to inspect.
"""
...
def _repo_root() -> str:
    return os.environ.get("SAD_WF_ROOT", ROOT)


def log_dir() -> str:
    return os.path.join(_repo_root(), ".workflow")
