...
(`.workflow/events.jsonl`, via tools/workflow_log.py) whenever:
...
Inspect with:  python tools/board.py logs --tool skill   (or command / file / bash)
...
# tools/<name>.py that already self-log via workflow_log -> don't double-count
SELF_LOGGING_TOOLS = ("tools/board.py", "tools/sync_board.py", "tools/workflow_log.py")
