...
// Expand input paths: a directory contributes its immediate `.csv` files; a file
// is taken as-is. Keeps ingestion to files already on disk (no fetch/scrape).
  ...
  for (const p of paths) {
    ...
    if (st.isDirectory()) {
      for (const entry of readdirSync(p).sort()) {
        if (extname(entry).toLowerCase() === '.csv') files.push(join(p, entry));
      }
    } else {
      files.push(p);
    }
...
// The filename stem, used as the ticker for the one-file-per-ticker layout when
// the row carries no ticker column/value. Normalized like any other ticker.
...
export function runImport(
  ...
  for (const file of files) {
    ...
    const fallbackTicker = fileStem(file);
