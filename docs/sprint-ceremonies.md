...
Tests: `tools/tests/test_board_sprint.py` (27 checks) + the existing
...
`tools/tests/test_board_sprint.py` (§5.6), and the `/sprint-retro` command + the four
...
### 5.6 Tests (`tools/tests/test_board_sprint.py`, extend)
...
(`docs/work-process-analysis.md` §10 STEP A / **#16**: `board.py idea-new`,
...
STEP A (#16) build steps are spec'd in `docs/work-process-analysis.md` §10 "STEP A".
...
python3 tools/board.py render                       # board.md shows the active sprint
# …build via /build-toward CAP-screen … then at close:
python3 tools/board.py sprint-close BATCH-NNN
