...
A `SessionStart` hook (a sibling of our existing `tools/hooks/*.py`) that shells out to
...
- **C-1 — `SessionStart` re-grounding hook _(do this first)_.** New `tools/hooks/session_ground.py`,
...
- **K-1 — OS cron / GitHub Actions → headless Claude Code.** A scheduled job runs
  `claude -p "/sm-health" --output-format json` (or `python tools/board.py metrics && … exceptions
...
of `tools/board.py` (24 subcommands), `tools/hooks/*`, `.claude/{commands,skills,agents}/*`,
